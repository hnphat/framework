...
class Apps_Class_Upload extends Apps_Class_Database
{
    ...
    public function copyFileToFolder($folderStart = "./resource/upload/", $nameFile = "", $folderEnd = "../resource/upload/") {
        $folder = $folderEnd. date("d-m-Y");
        if (!file_exists($folder)) {
            mkdir($folder, 0777, true);
        }
        return copy($folderStart.$nameFile,$folderEnd.$nameFile);
    }
}